...
# FIXES INCLUDED:
# - VRP v2 payload format: locations.location (required by Optimization v2)
# - Correct polyline decoder (was preventing route line + arrows)
# - Stable click-pin marker visibility
# - Slider min=max crash guard
# - Duplicate element id prevention with explicit keys
# - Replace deprecated use_container_width usage patterns where applicable
...
# CONFIG
# -----------------------------
NB_API_KEY = "a08a2b15af0f432c8e438403bc2b00e3"  # NOTE: better in st.secrets in production
...
def extract_route_geometry(resp: Any) -> List[Tuple[float, float]]:
    ...
    if isinstance(poly, str) and poly.strip():
        # Try precision=5 first; if empty, try 6 (some providers do)
        pts = decode_polyline(poly, precision=5)
        if not pts:
            pts = decode_polyline(poly, precision=6)
        return pts
...
def make_map(
    ...
    if route_pts and len(route_pts) >= 2:
        ...
        try:
            base = folium.PolyLine(route_pts, weight=0, opacity=0)
            base.add_to(m)
            PolyLineTextPath(base, "  ➤  ", repeat=True, offset=7, attributes={"font-size": "14", "fill": "black"}).add_to(m)
        except Exception:
            pass
...
def render_map(m: folium.Map, key: str) -> Dict[str, Any]:
    # st_folium width expects a number; let it flow in Streamlit layout by omitting width
    return st_folium(m, height=520, key=key)
...
with tabs[0]:
    ...
    if st.button("🧭 Geocode all missing coordinates (cached)", key="geo_geocode_btn"):
    ...
    st.dataframe(ss.stops_df, height=220)
...
with tabs[1]:
    ...
    if st.button("🎲 Generate random stops around center", key="pl_gen_random"):
    ...
    if st.button("Search Places (POIs)", key="pl_search_pois"):
        q = f"{kw}".strip()
        if not q:
...
with tabs[2]:
    ...
    if len(ss.stops_df) < 2:
        ...
        if len(stops_ll) < 2:
            st.warning("You have stops but not enough with lat/lng. Geocode first.")
        else:
            left, right = st.columns([1.15, 1.0])

            with left:
                st.markdown("### Step 1 — Directions (Before)")
                if st.button("🧭 Compute route (Before)", key="rt_before_btn"):
                    origins = stops_ll[:-1]
                    dests = stops_ll[1:]
            ...
            with right:
                ...
                if st.button("⚙️ Run optimization (VRP v2)", key="vrp_run_btn"):
                ...
                if st.button("🧭 Compute route (After)", key="rt_after_btn"):
                    ll_after = [latlng_str(float(r.lat), float(r.lng)) for r in df_after.dropna(subset=["lat", "lng"]).itertuples()]
                    if len(ll_after) < 2:
                        st.error("Not enough valid lat/lng stops to compute after-route.")
                    else:
...
with tabs[3]:
    ...
    if len(ss.stops_df) < 2:
        ...
        ll = [latlng_str(float(r.lat), float(r.lng)) for r in df_use.dropna(subset=["lat", "lng"]).itertuples()]
        if st.button("📐 Compute Distance Matrix (NxN)", key="mx_btn"):
        ...
        if isinstance(data, dict) and data.get("rows"):
            ...
            st.dataframe(pd.DataFrame(dist_km).round(2), height=240)
            st.markdown("### Duration (min)")
            st.dataframe(pd.DataFrame(dur_min).round(1), height=240)
...
with tabs[4]:
    ...
    with s1:
        ...
        if st.button("🧷 Snap to Road", key="snap_btn"):
    ...
    with s2:
        ...
        if st.button("🟦 Compute Isochrone", key="iso_btn"):
            params = {"key": NB_API_KEY}
            body = {
